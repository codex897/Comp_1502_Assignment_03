...
public class Menu {
	...
	public double checkNegativePrice(double price) throws NegativePriceException{
		 if(price<0.0) {
			throw new NegativePriceException("Price cannot be a negative number, try again");
		 }
		    return price;
		...
		while (true) {
	        try {
	            ...
	            if (Long.toString(snLong).length() == 10) {
					return Long.toString(snLong);
				} else if(!(Long.toString(snLong).length() == 10)) {
					System.out.println("Invalid input: must have 10 digits");
				}
